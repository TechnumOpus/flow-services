...
public class SapIntegrationService {
    ...
    private void processResponse(String responseBody, String location, String sku) {
        try {
            log.debug("Processing SAP response for location: {}, SKU: {}", location, sku);
            log.trace("Raw SAP response: {}", responseBody); // Use trace for full response to avoid log spam

            SapResponse sapResponse = objectMapper.readValue(responseBody, SapResponse.class);

            // Safely check for SAP errors without causing NPE
            boolean hasError = false;
            String errorMessage = "";

            try {
                if (sapResponse != null &&
                        sapResponse.getStockDetailResponse() != null &&
                    ...
                    if (returnItem.getMessage() != null && !returnItem.getMessage().trim().isEmpty()) {
                        log.warn("SAP returned message for location: {}, SKU: {}, Message: {}",
                                location, sku, returnItem.getMessage());
                        errorMessage = returnItem.getMessage();
                    }
                ...
                log.warn("Could not parse SAP return information for location: {}, SKU: {}, continuing with data processing",
                        location, sku);
            }

            // Don't process data if there's a confirmed error
            if (hasError) {
                log.error("Skipping data processing due to SAP error for location: {}, SKU: {}", location, sku);
                return;
            }
            ...
            try {
                if (sapResponse != null &&
                    ...
                    List<SapResponse.StockItem> items = sapResponse.getStockDetailResponse().getGtData().getItem();
                    log.info("Found {} stock items for location: {}, SKU: {}", items.size(), location, sku);

                    int processedCount = 0;
                    int validCount = 0;

                    for (SapResponse.StockItem item : items) {
                        processedCount++;
                        if (isValidItem(item)) {
                            validCount++;
                            saveDailyConsumptionLog(item, location, sku);
                        } else {
                            log.debug("Skipping invalid item at index {} for location: {}, SKU: {}",
                                    processedCount - 1, location, sku);
                        }
                    }

                    log.info("Processed {}/{} items successfully for location: {}, SKU: {}",
                            validCount, processedCount, location, sku);

                } else {
                    log.warn("No stock data structure found in SAP response for location: {}, SKU: {}", location, sku);
                }
            } catch (Exception e) {
                log.error("Error processing stock data for location: {}, SKU: {}", location, sku, e);
            }

        } catch (Exception e) {
            log.error("Error parsing SAP response for location: {}, SKU: {}", location, sku, e);
            log.debug("Problematic response body: {}", responseBody);
            ...
            if (e instanceof com.fasterxml.jackson.databind.exc.UnrecognizedPropertyException) {
                ...
                log.error("Unrecognized JSON property '{}' in class '{}'. Consider adding this field to the DTO or ensure @JsonIgnoreProperties(ignoreUnknown = true) is present.",
                        upe.getPropertyName(), upe.getReferringClass().getSimpleName());
            }
        }
    }
